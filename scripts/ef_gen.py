...
def runsim_from_ef(ef, seed):
    ...
    for i, item in enumerate(msdu_interval):
        ...
        ef_path = os.path.join('ef_src', ef_fn)
        ...
        with open(ef, 'U') as ef_tml, open(ef_path, 'w') as ef_new:
            for line in ef_tml:
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python %s <ef_template>") % sys.argv[0]
        sys.exit(1)
    ef_files = sys.argv[1:]
