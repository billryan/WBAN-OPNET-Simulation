...
static void wban_send_beacon_frame () {
	...
	SF.rap2_start2sec = SF.BI_Boundary + SF.rap2_start * SF.slot_length2sec;
...
static void wban_attempt_TX() {
	...
	if ((op_stat_local_read(TX_BUSY_STAT) == 1.0) || (OPC_FALSE == SF.ENABLE_TX_NEW) || (OPC_TRUE == SF.SLEEP)) {
		FOUT;
	}
...
static double wban_backoff_period_boundary_get() {
	...
	next_backoff_period_boundary = phase_start_timeG + backoff_period_index * pCSMASlotLength2Sec;
